...
const logger = new Logger("StatementRetrievalTool");
let vectorStore: PGVectorStore | null = null;

export const StatementRetrievalTool = new DynamicStructuredTool({
    name: "statement_retriever",
    description: "Consulta a base de dados de extratos bancários para recuperar os documentos mais relevantes para a pergunta do usuário, filtrados por tenantId.",
    schema: z.object({
    ...
    func: async ({ query, tenantId, type, categoria }: { query: string, tenantId: string, type: TipoTransacao, categoria?: TipoCategoria }): Promise<string> => {
        ...
        try {
            const retriever = vectorStore.asRetriever({
                filter: { tenantId: tenantId, type, categoria, mode: 'Transacoes' },
                verbose: true,
                k: 100
            });
            ...
            const pageContents = results.map(doc => `${doc.pageContent} - Categoria: ${doc.metadata['categoria']}, Tipo: ${doc.metadata['type']}}`);
